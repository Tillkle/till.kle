...
if cso_file and indi_file:
    # Load CSVs with embedded commas handled
    cso_df = pd.read_csv(cso_file, quotechar='"', skipinitialspace=True)
    indi_df = pd.read_csv(indi_file, quotechar='"', skipinitialspace=True)
    ...
    cso_plants['Quantity'] = (
        cso_plants['Quantity'].astype(str).str.replace(",", "", regex=False).str.strip()
    ).astype(float)

    indi_plants['Quantity'] = (
        indi_plants['Quantity'].astype(str).str.replace(",", "", regex=False).str.strip()
    ).astype(float)
    ...
    plant_result = pd.concat([plant_comparison, plant_total], ignore_index=True)
    ...
    cso_activities['Quantity'] = (
        cso_activities['Quantity'].astype(str).str.replace(",", "", regex=False).str.strip()
    ).astype(float)

    indi_activities['Quantity'] = (
        indi_activities['Quantity'].astype(str).str.replace(",", "", regex=False).str.strip()
    ).astype(float)
    ...
    indi_nonplant['Quantity'] = (
        indi_nonplant['Quantity'].astype(str).str.replace(",", "", regex=False).str.strip()
    ).astype(float)

    cso_nonplant['Quantity'] = (
        cso_nonplant['Quantity'].astype(str).str.replace(",", "", regex=False).str.strip()
    ).astype(float)
    ...
    np_comparison = pd.merge(
        cso_np_summary, indi_np_summary, on='Item', how='outer',
        suffixes=('_CSO', '_Individual')
    ).fillna(0)
